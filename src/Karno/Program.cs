...
namespace Karno
{
	internal class Program
	{
		...
		{
			static void PrintSeparator() => Console.WriteLine(new string('=', Console.WindowWidth));

			var map = new KMap(4, [0, 1, 2, 4, 5, 9, 10, 11, 13, 15], []);
			map.PrintCoverages(true);
			map.PrintTestResults();
			PrintSeparator();

			map = new KMap(4, [0, 2, 4, 5, 10, 11, 13, 15], []);
			map.PrintCoverages(true);
			map.PrintTestResults();
			PrintSeparator();
			...
			map = new KMap(3, [0, 1, 2, 3, 7], []);
			map.PrintCoverages(true);
			map.PrintTestResults();
			PrintSeparator();

			map = new KMap(4, [1, 11, 12, 13, 14, 15], [3, 4, 5, 9]);
			map.PrintCoverages(true);
			map.PrintTestResults();
			PrintSeparator();

			map = new KMap(5, [2, 5, 7, 8, 10, 13, 15, 17, 19, 21, 23, 24, 29, 31], []);
			map.PrintCoverages(true);
			map.PrintTestResults();
			PrintSeparator();
		}
